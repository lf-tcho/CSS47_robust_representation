...
class preactblock(torch.nn.Module):
    ...
    def __init__(self,in_planes,planes,stride=1):
        ...
        self.conv1=torch.nn.Conv2d(in_planes,planes,stride=1,kernel_size=3,padding=1,bias=False)
        self.bn2=torch.nn.BatchNorm2d(planes)
        self.conv2=torch.nn.Conv2d(planes,planes,stride=1,kernel_size=3,padding=1,bias=False)

        self.shortcut=torch.nn.Sequential()
        if stride!=1 or in_planes!=self.expansion*planes:
            self.shortcut=torch.nn.Sequential(torch.nn.Conv2d(in_planes,self.expansion*planes,kernel_Size=1,stride=1,bias=False))
...
class bottleneck(torch.nn.Module):
    ...
    def __init__(self,in_planes,planes,stride=1):
        ...
        self.conv1=torch.nn.Conv2d(in_planes,planes,kernel_size=1,stride=1,padding=1,bias=False)
        self.bn1=torch.nn.BatchNorm2d(planes)
...
class preact_bottelneck(torch.nn.Module):
    ...
    def __init__(self,in_planes,planes,stride=1):
        ...
        self.conv3=torch.nn.Conv2d(planes,self.expansion*planes,kernel_size=3,stride=1,padding=1,bias=False)
